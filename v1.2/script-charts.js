...
function pieChart() {
  ...
  var sliceBorderStyle = "#000";
  var sliceGradientColour = "#ddd";
  var maxPullOutDistance = 25;
  var pullOutFrameStep = 1;
  var pullOutFrameInterval = 40;
  var pullOutLabelPadding = 45;
  var pullOutLabelFont = "bold 14px Georgia";
  var pullOutValueFont = "bold 14px Georgia";
  var pullOutValuePrefix = "";
  var pullOutShadowColour = "rgba( 0, 0, 0, 0.4 )";
  var pullOutShadowOffsetX = 10;
  var pullOutShadowOffsetY = 10;
  var pullOutShadowBlur = 10;
  var pullOutBorderWidth = 2;
  var pullOutBorderStyle = "#333";
  var chartStartAngle = -.5 * Math.PI;
  var canvas = [];
  var currentPullOutSlice = [($('#chart').attr('data-slice') ? parseInt($('#chart').attr('data-slice')) : 0)];
  var activeSlice = [-1];
  var currentPullOutDistance = [15];
  var animationId = [0];
  var chartData = [[]];
  var chartColours = [[]];
  var totalValue = [0];
  ...
  function init(id) {
    ...
    drawChart();
    $('#'+chartById[chartId]).mousemove ( handleChartClick );
  }
      ...
      if ( distanceFromCentre <= chartRadius[chartId] ) {
        ...
        for ( var slice in chartData[chartId] ) {
          if ( clickAngle >= chartData[chartId][slice]['startAngle'] && clickAngle <= chartData[chartId][slice]['endAngle'] ) {
            if (activeSlice[chartId] != slice)
              toggleSlice ( slice );
            activeSlice[chartId] = slice;
            return;
          }
        }
      }
  ...
  function drawSlice ( context, slice ) {
    ...
    if ( slice == currentPullOutSlice[chartId] ) {
      ...
      context.fillText( pullOutValuePrefix + chartData[chartId][slice]['value'] + " %", centreX[chartId] + Math.cos(midAngle) * ( chartRadius[chartId] + maxPullOutDistance + pullOutLabelPadding ), centreY[chartId] + Math.sin(midAngle) * ( chartRadius[chartId] + maxPullOutDistance + pullOutLabelPadding ) + 20 );
